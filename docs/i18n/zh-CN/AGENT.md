...
│   └── CODE_OF_CONDUCT.md
├── examples/
├── packages/                      # JS 侧包(bffi-loader)
├── bin/                           # cli 工具(bffi codegen)
└── scripts/
```
...
| 异步         | `#[bffi_async]`:spawn-шим返回任务句柄;N-воркерный执行器;协作式取消 + 超时;解析经 event-loop enqueue;tokio opt-in;标签 0x0500-0x05FF |
