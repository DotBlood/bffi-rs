...
| Async                  | `#[bffi_async]`: spawn-шим возвращает хендл задачи; N-воркерный executor поллит future'ы; отмена (кооперативный drop) и таймауты; резолв доставляется через event-loop на JS-потоке; tokio - opt-in фича; дескрипторы `Promise<T>` |
| Лицензия               | MIT                                                       |
