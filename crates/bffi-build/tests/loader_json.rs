...
//! (`bffi_build::loader_json`): deterministic bytes, a committed
...
fn rendering_is_deterministic() {
    ...
    assert_eq!(
        bffi_build::loader_json::to_json(&module),
        bffi_build::loader_json::to_json(&module),
...
fn golden_matches_the_committed_file() {
    let rendered = bffi_build::loader_json::to_json(&fixture_module());
...
///
/// ```sh
/// BFFI_LOADER_JSON_OVERWRITE=1 cargo test -p bffi-build --test loader_json write_golden
/// ```
///
...
fn strings_survive_escaping() {
    ...
    let rendered = bffi_build::loader_json::to_json(&module);
    let parsed: serde_json::Value = serde_json::from_str(&rendered).expect("valid JSON");
    assert_eq!(
...
fn write_to_file_persists_the_bytes_verbatim() {
    ...
    let module = fixture_module();
    bffi_build::loader_json::write_to_file(&module, &path).expect("write succeeds");
    let from_disk = std::fs::read_to_string(&path).expect("the written file exists");
    assert_eq!(bffi_build::loader_json::to_json(&module), from_disk);
    // A second write is byte-identical (overwrite, no drift).
    bffi_build::loader_json::write_to_file(&module, &path).expect("rewrite succeeds");
...
fn write_to_file_surfaces_io_errors() {
    ...
    let result = bffi_build::loader_json::write_to_file(&fixture_module(), &target);
