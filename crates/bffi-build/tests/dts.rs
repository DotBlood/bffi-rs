...
//! (`bffi_build::dts::write_to_file`): the bytes on disk are exactly
...
fn written_bytes_are_exactly_the_render_output() {
    ...
    bffi_build::dts::write_to_file(&add_module(), &path).expect("write succeeds");
    let on_disk = std::fs::read(&path).expect("the file exists");
    assert_eq!(
        on_disk,
        bffi_dts::render(&add_module()).as_bytes(),
...
fn missing_parent_directories_are_created() {
    ...
    bffi_build::dts::write_to_file(&add_module(), &path)
...
fn an_existing_file_is_overwritten() {
    ...
    bffi_build::dts::write_to_file(&add_module(), &path).expect("first write succeeds");
    bffi_build::dts::write_to_file(&ping_module(), &path).expect("second write succeeds");
    let on_disk = std::fs::read_to_string(&path).expect("the file exists");
    assert_eq!(
        on_disk,
        bffi_dts::render(&ping_module()),
...
fn repeated_writes_are_byte_identical() {
    ...
    bffi_build::dts::write_to_file(&add_module(), &first).expect("first write succeeds");
    bffi_build::dts::write_to_file(&add_module(), &second).expect("second write succeeds");
...
fn io_errors_surface_unchanged() {
    ...
    let result = bffi_build::dts::write_to_file(&add_module(), &as_directory);
    assert!(
        result.is_err(),
