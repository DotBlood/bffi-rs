...
//! (`bffi_callback::abi`): wire encoding/decoding round-trips, the
...
use bffi_build::runtime::{buffer_len, buffer_ptr, free_buffer};
use bffi_callback::abi::{bind_body, decode_args, encode_result, invoke_body, revoke_body};
use bffi_callback::{CallbackSig, Value, ValueType, register};
use bffi_core::ErrorCode;
use bffi_core::take_last_error;
use bffi_types::wire;
...
fn bind_body_stores_the_signature_and_pointer() {
    ...
    let info = bffi_callback::js_callback(bffi_core::Handle::from_raw(slot))
...
fn invoke_body_returns_the_result_through_the_buffer_table() {
    ...
    let len = buffer_len(bffi_core::Handle::from_raw(slot));
    assert_eq!(len, 5, "one i32 wire record");
    let ptr = buffer_ptr(bffi_core::Handle::from_raw(slot));
    assert!(!ptr.is_null());
    ...
    assert!(free_buffer(bffi_core::Handle::from_raw(slot)));
...
fn invoke_body_maps_dead_handles_to_invalid_handle() {
    ...
    .expect("table has room");
    assert!(bffi_callback::revoke(handle));
