...
use bffi_callback::{CallbackSig, Value, ValueType, invoke, register, set_js_thread};
use bffi_event_loop::marshal;
    ...
    JS_THREAD.get_or_init(|| {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let executed = bffi_event_loop::run();
        ...
        while !bffi_event_loop::is_running() {
            assert!(
...
fn marshal_delivers_invoke_onto_the_js_thread() {
    ...
    assert_eq!(
        dead,
        Err(bffi_callback::CallbackError::InvalidHandle(handle))
    );
}
...
fn wrong_thread_invoke_outside_the_loop_maps_to_code_12() {
    ...
    assert_eq!(invoked, Err(bffi_callback::CallbackError::WrongThread));

    let converted = bffi_core::BffiError::from(invoked.unwrap_err());
    assert_eq!(converted.code, bffi_core::ErrorCode::WrongThread);
