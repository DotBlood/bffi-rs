...
//!   [`bffi::dts::ModuleDef`] with the deterministic
//!   [`bffi::dts::render`] and writes the bytes to disk in one call
...
//! let handle = bffi::build::runtime::store_bytes(CopiedBuf::from_slice(b"payload"))
//!     .expect("buffer table has room");
//! assert!(!bffi::build::runtime::buffer_ptr(handle).is_null());
//! assert_eq!(bffi::build::runtime::buffer_len(handle), 7);
