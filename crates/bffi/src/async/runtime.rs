...
//! [`spawn_on_tokio`](super::spawn_on_tokio); completion is recorded on
...
use super::executor::{self, BoxFuture};
use super::task;

/// Hard cap on tokio worker threads.
const MAX_TOKIO_WORKERS: usize = 8;

fn runtime() -> Result<&'static tokio::runtime::Runtime, super::AsyncError> {
    ...
    match initialized {
        ...
        Err(message) => {
            ...
            Err(super::AsyncError::RuntimeUnavailable)
        }
    }
}

/// The executor mode selected with
/// [`set_executor_kind`](super::set_executor_kind).
