...
| `Result<T, E>`                 | like `T`; on `Err` returns `ErrorCode::DomainError` (13) | the error message is `E`'s `Display`, stored in the last error with `E` as the source; JS reads the source as `Error.cause` (┬з5) |
