...
use bffi::bffi_async::{AsyncValue, cancel, pending_tasks, spawn_on_tokio};
...
fn executor_mode_switch_routes_spawn_to_tokio() {
    bffi::bffi_async::set_executor_kind(bffi::bffi_async::ExecutorKind::Tokio);
    let done = Arc::new(AtomicBool::new(false));
    let done2 = Arc::clone(&done);
    bffi::bffi_async::spawn(async move {
        done2.store(true, Ordering::SeqCst);
        ok(AsyncValue::Unit)
    })
    .expect("room");
    ...
    bffi::bffi_async::set_executor_kind(bffi::bffi_async::ExecutorKind::Internal);
