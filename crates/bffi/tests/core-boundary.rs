...
use bffi::bffi_core::error::ErrorCode;
use bffi::bffi_extern;
...
fn extern_bodies_can_signal_failures_through_codes() {
    let code = bffi::bffi_core::run_extern_body(|| ErrorCode::InvalidHandle);
    assert_eq!(code, ErrorCode::InvalidHandle);
    assert!(bffi::bffi_core::take_last_error().is_none());
}

#[test]
fn run_extern_body_converts_panics_into_codes_and_last_error() {
    let code = bffi::bffi_core::run_extern_body(|| panic!("outer boom"));
    assert_eq!(code, ErrorCode::Panic);

    let error = bffi::bffi_core::take_last_error().expect("last error must be stored");
    assert_eq!(error.code, ErrorCode::Panic);
    assert_eq!(error.message, "outer boom");
    assert!(
        bffi::bffi_core::take_last_error().is_none(),
...
fn last_error_stays_thread_local_across_the_boundary() {
    let code = bffi::bffi_core::run_extern_body(|| panic!("main-thread only"));
    assert_eq!(code, ErrorCode::Panic);

    let seen_from_thread = std::thread::spawn(bffi::bffi_core::take_last_error)
        .join()
        .expect("thread must not panic");
    assert!(seen_from_thread.is_none(), "other threads see no error");

    let own = bffi::bffi_core::take_last_error().expect("owner thread sees the error");
