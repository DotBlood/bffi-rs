...
use bffi::bffi_event_loop::{enqueue, executed_total, marshal, pending, pump, run, stop};
...
fn loop_lifecycle() {
    ...
    let (err_tx, err_rx) = std::sync::mpsc::channel::<bffi::bffi_core::BffiError>();
    enqueue(Box::new(|| panic!("loop boom"))).expect("not stopped");
    enqueue(Box::new(move || {
        let error = bffi::bffi_core::take_last_error().expect("the panic must be stored");
    ...
    let panic_error = err_rx
        ...
        .expect("drained");
    assert_eq!(panic_error.code, bffi::bffi_core::ErrorCode::Panic);
...
fn marshal_without_runner_is_not_running() {
    ...
    if bffi::bffi_event_loop::is_running() {
