...
//!
//! Same scheme as `bffi-macros` (`bffi[E0XX]:` + ` = help: ` +
//! ` = note: `); the numbering CONTINUES after the frozen `E001`-`E004`
//! of `#[bffi]` - do not renumber either series (the UI goldens in
//! `tests/ui` lock them):
//!
//! | Code   | Meaning                                                  |
//! |--------|----------------------------------------------------------|
...
/// One structured `bffi-class` diagnostic; rendered to a
/// `compile_error!` with the shared `bffi[<code>]:` format.
...
impl MacroDiagnostic {
    ...
    /// Renders into a `syn::Error` anchored at `span`.
    // Consuming `self` mirrors the bffi-macros renderer contract.
    ...
    pub(crate) fn class_shape(span: Span, what: &str) -> syn::Error {
        Self::new("E005", format!("unsupported class shape: {what}"))
    ...
    pub(crate) fn tag(span: Span, what: impl Into<String>) -> syn::Error {
        Self::new(
            "E006",
            format!("invalid class tag: {}", what.into()),
        )
    ...
    pub(crate) fn method_shape(span: Span, what: &str) -> syn::Error {
        Self::new("E007", format!("unsupported member shape: {what}"))
            .with_help(format!(
    ...
    pub(crate) fn impl_binding(span: Span, what: &str) -> syn::Error {
        Self::new("E008", format!("invalid impl binding: {what}"))
