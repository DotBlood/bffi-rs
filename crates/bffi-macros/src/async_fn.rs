...
pub(crate) fn expand(model: &AsyncFnModel) -> TokenStream {
            ...
            AsyncParam::OwnedStr => {
                ...
                conversions.extend(quote! {
                    ...
                    let #ident = unsafe { ::std::ffi::CStr::from_ptr(#ptr) }
                        .to_bytes();
                    let #ident = match ::bffi_types::unsafe_zero_copy::str_view(#ident) {
    ...
    let tail = async_tail(&model.ret, call);
    let future = quote! {
        let __fut = async move {
            #tail
        };
        match ::bffi_async::spawn(Box::pin(__fut)) {
            Ok(handle) => {
...
fn async_tail(ret: &RetKind, call: TokenStream) -> TokenStream {
    match ret {
        RetKind::Unit => quote! {
            #call.await;
            ::std::result::Result::Ok(::bffi_async::AsyncValue::Unit)
        },
        RetKind::Result(inner) => {
            let ok_tail = async_value_from(inner);
            quote! {
                match #call.await {
                    ...
                    ::std::result::Result::Err(__err) => {
                        ::std::result::Result::Err(::bffi_core::BffiError::with_source(
        ...
        other => {
            let conversion = async_value_from(other);
            quote! {
...
/// `__value`.
fn async_value_from(ret: &RetKind) -> TokenStream {
    match ret {
        RetKind::Unit => quote! { ::bffi_async::AsyncValue::Unit },
        _ => quote! { ::bffi_async::AsyncValue::from(__value) },
    }
}
