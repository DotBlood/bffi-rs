...
import { assertSchema, buildDeclarations, type FunctionJson, type ModuleJson, type TsName } from "./loader.ts";
 ...
 */
export function createApi<J extends ModuleJson>(json: J, libraryPath: string): ApiOf<J> {
  assertSchema(json);
  const { symbols } = dlopen(libraryPath, buildDeclarations(json));
