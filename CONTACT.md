# Contact

[English](https://github.com/z2net/bffi-rs/blob/main/CONTACT.md) | [Русский](https://github.com/z2net/bffi-rs/blob/main/docs/i18n/ru/CONTACT.md) | [简体中文](https://github.com/z2net/bffi-rs/blob/main/docs/i18n/zh-CN/CONTACT.md)

## Maintainers

- **@z2net** - project author & lead

## Contributors

- **ZCode (GLM, [Z.ai](https://z.ai))** - AI agent: developer infrastructure, docs, i18n

## Getting in touch

| Purpose              | Channel                                                        |
|----------------------|----------------------------------------------------------------|
| Bug reports          | [GitHub Issues](https://github.com/z2net/bffi-rs/issues)    |
| Feature requests     | [GitHub Issues](https://github.com/z2net/bffi-rs/issues)    |
| Security (private!)  | contact@z2net.com                                              |
| General questions    | [Discussions](https://github.com/z2net/bffi-rs/discussions) |

## Security

Please report vulnerabilities **privately** to contact@z2net.com.
Do not open public issues for security matters.

See [SECURITY.md](https://github.com/z2net/bffi-rs/blob/main/SECURITY.md) for disclosure policy.
